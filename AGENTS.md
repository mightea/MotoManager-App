...
- `Views/MotorcycleSummaryHeader.swift` — immersive photo header (Dynamic Type-scaled height) with the Wechseln pill and the add/gear actions; `UI/MotorcycleWorkspace.swift` stacks it above the tab content, lays it over the content; a list that adopted `.tracksWorkspaceHeader()` starts beneath it and collapses it scroll-linked (`UI/WorkspaceHeaderState.swift`), and on compact widths records are pushed full screen over it
