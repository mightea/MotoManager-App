...
- **Phase 3 — implemented, then removed by decision.** The Fuel/Service `NavigationSplitView` layout was built (commit `e03977f`) but broke the app's visual design on iPad, so it was reverted (`420ccd6`). All tabs keep push navigation on every device; the detail pages read fine at the 700 pt content column. Don't reintroduce a split layout without a design pass first.
